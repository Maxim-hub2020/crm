    ...
    <div className="space-y-4">
      ...
      <section className="rounded-[24px] border border-slate-200 bg-white p-3 sm:p-4">
        <div className="flex gap-2 overflow-x-auto pb-3 [scrollbar-width:none]">
          {form.rooms.map((room) => room.id === activeRoom?.id
            ? <div key={room.id} className="flex shrink-0 items-center rounded-full bg-slate-900 px-2 text-white">
              <input value={room.name} onChange={(event) => updateActiveRoom({ name: event.target.value })} aria-label="Название комнаты" className="w-32 bg-transparent px-2 py-2 text-sm font-bold text-white outline-none placeholder:text-slate-400" placeholder="Название комнаты" />
              {form.rooms.length > 1 ? <button type="button" onClick={() => removeRoom(room.id)} title="Удалить комнату" aria-label="Удалить комнату" className="rounded-full p-1.5 text-slate-300 hover:bg-white/10 hover:text-white"><Trash2 size={14} /></button> : null}
            </div>
            : <button key={room.id} type="button" onClick={() => setActiveRoomId(room.id)} className="shrink-0 rounded-full bg-slate-100 px-4 py-2 text-sm font-bold text-slate-600 transition hover:bg-slate-200">{room.name}</button>)}
